...
.stApp { background: #0D0F14; color: #E8E0D0; }
...
.card-unit { font-size: 0.8rem; color: #5A5F70; margin-left: 0.3rem; }
...
.score-row { display: flex; align-items: center; margin-bottom: 0.7rem; gap: 0.8rem; }
.score-label {
    font-size: 0.65rem;
...
.score-val { font-size: 0.7rem; color: #C9A84C; width: 30px; text-align: right; }
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                ...
                {'range': [min_price + (max_price - min_price) * 0.33,
                           min_price + (max_price - min_price) * 0.66], 'color': '#1C2030'},
...
def compute_scores(area, bedrooms, bathrooms, airconditioning,
    ...
    comfort    = sum([airconditioning == 'yes', hotwaterheating == 'yes',
                      basement == 'yes', guestroom == 'yes'])
...
with st.sidebar:
    ...
    area = st.slider("Surface (m²)", 50, 500, 150, step=10)
...
if not predict_clicked:
    ...
    with col1:
        ...
        with c1:
            st.markdown(
                f'<div class="card"><div class="card-label">Surface</div>'
                f'<div class="card-value">{area:,}<span class="card-unit">m²</span></div></div>',
                unsafe_allow_html=True)
        with c2:
            st.markdown(
                f'<div class="card"><div class="card-label">Chambres</div>'
                f'<div class="card-value">{bedrooms}<span class="card-unit">ch.</span></div></div>',
                unsafe_allow_html=True)
        with c3:
            st.markdown(
                f'<div class="card"><div class="card-label">Étages</div>'
                f'<div class="card-value">{stories}<span class="card-unit">niv.</span></div></div>',
                unsafe_allow_html=True)

        st.markdown(render_chips(chips_data), unsafe_allow_html=True)

    with col2:
        st.markdown('<div class="card-label" style="margin-bottom:1rem">Analyse du bien</div>',
                    unsafe_allow_html=True)
        st.markdown(render_bars(scores), unsafe_allow_html=True)
    ...
    with st.spinner("Calcul en cours..."):
        try:
            result = session.sql(f"""
                SELECT HOUSE_PRICE_DB.HOUSE_SCHEMA.HOUSE_PRICE_VOTING!PREDICT(
            ...
            with col1:
                ...
                st.plotly_chart(make_gauge(price), use_container_width=True,
            ...
            with col2:
                st.markdown(
                    '<div class="card-label" style="margin-bottom:1rem">Facteurs de valorisation</div>',
                    unsafe_allow_html=True)
                st.markdown(render_bars(scores), unsafe_allow_html=True)
                ...
                st.markdown(
                    '<div class="card-label" style="margin-bottom:0.5rem">Équipements</div>',
                    unsafe_allow_html=True)
                st.markdown(render_chips(chips_data), unsafe_allow_html=True)
                ...
                st.markdown("""
                <div class="info-box">
                    Estimation générée par le modèle
                    <strong style="color:#C9A84C">VotingRegressor</strong>
